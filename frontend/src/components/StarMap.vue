  ...
  sel.enter().append('circle')
    ...
    .on('mouseenter', function (this: SVGCircleElement, event: MouseEvent, d: unknown) {
      ...
      tooltip!.style('display', 'block').style('left', lx + 'px').style('top', ly + 'px')
        .html(`<div style="font-size:11px;line-height:1.5;max-width:200px"><strong style="color:${typeColors[a.type]||'#9ca3af'}">${a.type}</strong><br><span style="color:#6b7280">con:</span> ${con||'—'}<br><span style="color:#6b7280">ID:</span> ${a.alertId}<br><span style="color:#6b7280">RA:</span> ${hh}h ${mm}m ${ss}s<br><span style="color:#6b7280">Dec:</span> ${decD}° ${Math.abs(decM)}' ${decS}"<br><span style="color:#6b7280">mag:</span> ${a.magnitude.toFixed(1)}<br><span style="color:#6b7280">z:</span> ${a.redshift.toFixed(3)}<br><span style="color:#6b7280">rise:</span> ${a.riseTime.toFixed(2)}s<br><span style="color:#6b7280">score:</span> ${a.score.toFixed(2)}<br><span style="color:#6b7280">age:</span> ${age}s</div>`)
    })
